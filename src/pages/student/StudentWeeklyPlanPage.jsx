    ...
    <div>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:20, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <button onClick={() => setWeekStart(getMonday(new Date(new Date(weekStart).setDate(new Date(weekStart).getDate()-7))))}
            style={navBtn}><i className="fas fa-chevron-left"></i></button>
          <div style={{ padding:'8px 16px', background:'#f8fafc', borderRadius:8, fontSize:13, fontWeight:600, color:'#475569', minWidth:200, textAlign:'center' }}>
            {weekLabel}
          </div>
          <button onClick={() => setWeekStart(getMonday(new Date(new Date(weekStart).setDate(new Date(weekStart).getDate()+7))))}
            style={navBtn}><i className="fas fa-chevron-right"></i></button>
        </div>
                ...
                <div style={{ display:'flex', gap:8 }}>
                  {MAT_TYPES.map(type => (
                    <label key={type} style={{ fontSize:12, display:'flex', alignItems:'center', gap:4, cursor:'pointer' }}>
                      <input type="checkbox" checked={(plan.material_types??[]).includes(type)} onChange={() => toggleType(idx, type)} />
                      {type}
                    </label>
                  ))}
                </div>
...
const navBtn = { padding:'8px 12px', border:'1.5px solid #e2e8f0', borderRadius:8, background:'#fff', cursor:'pointer', color:'#475569' };
