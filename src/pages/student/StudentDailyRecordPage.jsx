...
export default function StudentDailyRecordPage() {
  ...
  async function save() {
    ...
    const payloads = drafts.filter(d => d.subject).map(d => ({
      student_id: student.id, record_date: date,
  ...
  function startEdit(r) {
    setEditingId(r.id);
    setEditData({ subject:r.subject, material:r.material??'', study_range:r.study_range??'', study_minutes:r.study_minutes??0 });
  }
                ...
                <div style={{ background:colorMap[COLORS[i%COLORS.length]], padding:'10px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  ...
                  <div style={{ display:'flex', gap:6 }}>
                    {editingId === r.id ? (
                      ...
                      <>
                        <button onClick={() => startEdit(r)} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.8)', cursor:'pointer' }}><i className="fas fa-pen"></i></button>
                        <button onClick={() => remove(r.id)} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.8)', cursor:'pointer' }}><i className="fas fa-trash"></i></button>
                      </>
                    )}
                  </div>
                </div>
